...
if (isset($_POST['accion']) && $_POST['accion'] == 'insertar') {
    ...
    $query = "INSERT INTO armas (nombre, tipo, daño_base, escalado) VALUES (?, ?, ?, ?)";
    $stmt = $conexion->prepare($query);
    $stmt->bind_param("ssss", $nombre, $tipo, $daño_base, $escalado);
    $stmt->execute();
    $stmt->close();
}
...
if (isset($_POST['accion']) && $_POST['accion'] == 'eliminar') {
    $id = $_POST['id'];
    $query = "DELETE FROM armas WHERE id = ?";
    $stmt = $conexion->prepare($query);
...
if (isset($_POST['accion']) && $_POST['accion'] == 'actualizar') {
    ...
    $query = "UPDATE armas SET nombre = ?, tipo = ?, daño_base = ?, escalado = ? WHERE id = ?";
    $stmt = $conexion->prepare($query);
    $stmt->bind_param("ssssi", $nombre, $tipo, $daño_base, $escalado, $id);
    $stmt->execute();
    $stmt->close();
}
...
$armas = $resultado->fetch_all(MYSQLI_ASSOC);

// Obtener arma a actualizar
$arma_para_actualizar = null;
if (isset($_GET['editar'])) {
    $id = $_GET['editar'];
    $query = "SELECT * FROM armas WHERE id = ?";
    $stmt = $conexion->prepare($query);
    $stmt->bind_param("i", $id);
    $stmt->execute();
    $resultado = $stmt->get_result();
    $arma_para_actualizar = $resultado->fetch_assoc();
    $stmt->close();
}
?>
